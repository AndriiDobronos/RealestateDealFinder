...
function formatMoney(value: number, currency?: string): string {
  ...
  if (code === 'UAH' || code === 'USD' || code === 'EUR') return new Intl.NumberFormat('uk-UA', { style: 'currency', currency: code, maximumFractionDigits: 0 }).format(value)
  return `${number.format(value)} ${currency || 'валюта не вказана'}`
}
function formatPerMeter(value: number, currency?: string): string {
  const code = normalizeCurrency(currency)
